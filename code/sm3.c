...
/* SM3 初始值 */
static const u32 g_sm3InitValues[8] = {
    0x7380166FUL, 0x4914B2B9UL, 0x172442D7UL, 0xDA8A0600UL,
    0xA96F30BCUL, 0x163138AAUL, 0xE38DEE4DUL, 0xB0FB0E4EUL
};

/* SM3 常数 T(j) */
static const u32 g_sm3Constants[64] = {
    0x79CC4519UL, 0xF3988A32UL, 0xE7311465UL, 0xCE6228CBUL, 0x9CA4F5A0UL, 0xAB2EFF1DUL, 0x83CD5650UL, 0x36222E44UL,
...
static void SM3_ProcessBlock(const u8 blockData[64], SM3_Context_S* context)
{
    ...
    for (int i = 16; i < 68; ++i) {
        messageWordW[i] = SM3_P1(messageWordW[i - 16] ^ messageWordW[i - 9] ^ SM3_LeftRotate(messageWordW[i - 3], 15)) ^
                         SM3_LeftRotate(messageWordW[i - 13], 7) ^ messageWordW[i - 6];
    }
    ...
    for (int round = 0; round < 64; ++round) {
        ...
        u32 tt1Value = (SM3_SelectFunction(round, stateValues[0], stateValues[1], stateValues[2]) + stateValues[3] + ss2Value + messageWordW1[round]) & 0xFFFFFFFFUL;
